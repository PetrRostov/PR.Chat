...
namespace PR.Chat.Infrastructure.ExpressionSerialization
{
    public partial class ExpressionSerializer
    {
        ...
        private const int TryParseArgumentsQuantity = 2;
        /*
       * DESERIALIZATION 
       */
        ...
        private object ParseConstantFromElement(XElement xml, string elemName, Type expectedType)
        {
            ...
            var tryParseMethod = expectedType.GetMethod("TryParse", BindingFlags.Public | BindingFlags.Static);
            if (tryParseMethod != null && tryParseMethod.GetParameters().Length == TryParseArgumentsQuantity)
            {
                var parseParameters = new object[] {objectStringValue, null};
                if ((bool)tryParseMethod.Invoke(null, parseParameters))
                    return parseParameters[1];
            }
